...
const Project = sequelize.define(
    'Project',
    {
        ...
        project_number: Sequelize.INTEGER,
...
app.post('/api/project', (req, res) => {
    ...
    if (req.session.user && req.session.user.role === 'admin') {
        ...
        Project.create(input_project)
            ...
            .catch(error => {
                let errorMessage = 'Encounter an error';
                if (error.errors && error.errors.length > 0) {
                    errorMessage = error.errors[0].message;
                }
                return res.send({error: errorMessage}, 400);
            })
...
app.post('/api/questionnarie', (req, res) => {
    ...
    if (req.body.search.questionnarie && req.body.search.questionnarie.length > 0) {
        ...
        if (req.body.search.searchTerm) {
            sql += ` AND ( Section LIKE '%${req.body.search.searchTerm}%' 
                           OR \`Variable name\` LIKE '%${req.body.search.searchTerm}%'  
                           OR description LIKE '%${req.body.search.searchTerm}%' ) `
        }
    } else {
        sql += " AND 1 = 0 ";
    }
...
app.post('/api/topic', (req, res) => {
    ...
    if (req.body.search.questionnarie && req.body.search.questionnarie.length > 0) {
        ...
        if (req.body.search.searchTerm) {
            sql += ` AND ( topic_name LIKE '%${req.body.search.searchTerm}%' 
                           OR variable LIKE '%${req.body.search.searchTerm}%'
                           OR description LIKE '%${req.body.search.searchTerm}%' ) `
        }
    } else {
        sql += " AND 1 = 0 ";
...
app.post('/api/topic_search', (req, res) => {
    ...
    if (req.body.searchTerm) {
        sql += ` AND ( topic_name LIKE '%${req.body.searchTerm}%' 
                       OR variable LIKE '%${req.body.searchTerm}%'
                       OR description LIKE '%${req.body.searchTerm}%' ) `
    }
        ...
        let task_sql =
                `
                  SELECT count(*) as total 
                    FROM task 
                   WHERE NOT status='The data was generated.'
                     AND DATETIME('now') < DATETIME(start_time, '+3 minutes') 
                `;
        console.log("---------------------------------------------------");
        console.log(task_sql);
        db.all(task_sql, function (err, rows) {
            let task_total = 0;
            rows.forEach(function (row) {
                console.log("task status: " + JSON.stringify(row));
                task_total = row.total;
            })
